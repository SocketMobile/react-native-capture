#import "NativeCaptureModule.h"
#import "SktEvent.h"
#import <CaptureSDK/CaptureSDK.h>
#import <React/RCTUtils.h>

@interface NativeCaptureModule () <SKTCaptureDelegate> {
    SKTCapture* _capture;
    bool _captureOpened;
...
@implementation NativeCaptureModule

// to export a module named NativeCaptureModule
...
{
   @try{
        id rpc = [NativeCaptureModule ParseFromJson:jsonRpc];
...
-(void)callMethodFromJsonRpc:(NSDictionary*)jsonRpc completionHandler:(void(^)(NSString* response)) block {
    ...
    if([method  isEqual: @"openclient"]){
        ...
        //     block([NativeCaptureModule returnJsonRpcWithError:SKTCaptureE_ALREADYDONE withMessage:@"Capture already open" withId:jsonRpcId]);
        //     return;
        // }
        ...
        [_capture openWithAppInfo:appInfo completionHandler:^(SKTResult result) {
            ...
            if(SKTSUCCESS(result)){
                ...
                response = [NativeCaptureModule returnJsonRpcWithHandle:handle withId:jsonRpcId];
            }
            else{
                response = [NativeCaptureModule returnJsonRpcWithError:result withMessage:@"failed to open Capture" withId:jsonRpcId];
                _captureOpened = false;
            }
            if(block != nil){
                block(response);
            }
            [openCompleteEvent setEvent];
        }];
        ...
        if(handle == (int) _capture){
            [_capture openDeviceWithGuid:guid completionHandler:^(SKTResult result, SKTCapture *deviceCapture) {
                ...
                if(SKTSUCCESS(result)){
                    ...
                    response = [NativeCaptureModule returnJsonRpcWithHandle:handleValue withId:jsonRpcId];
                }
                else{
                    response = [NativeCaptureModule returnJsonRpcWithError:result withMessage:@"failed to open the device" withId:jsonRpcId];
                }
                if(block != nil){
                    block(response);
                }
            ...
            if(block != nil){
                ...
                NSString* response = [NativeCaptureModule returnJsonRpcWithError:result withMessage:@"handle is not the main Capture object" withId:jsonRpcId];
                block(response);
            }
        }
    }
        ...
        if(handle == (int)_capture){
            [_capture closeWithCompletionHandler:^(SKTResult result) {
                ...
                if(SKTSUCCESS(result)){
                    response = [NativeCaptureModule returnJsonRpcWithId:jsonRpcId];
                }
                else{
                    response = [NativeCaptureModule returnJsonRpcWithError:result withMessage:@"failed to close Capture" withId:jsonRpcId];
                }
                _captureOpened = false;
                if(block != nil){
                    block(response);
            ...
            [device closeWithCompletionHandler:^(SKTResult result) {
                ...
                if(SKTSUCCESS(result)){
                    ...
                    response = [NativeCaptureModule returnJsonRpcWithError:result withMessage:@"failed to close the device" withId:jsonRpcId];
                }
                if(block != nil){
                    block(response);
                }
            ...
            if(block != nil){
                ...
                NSString* response = [NativeCaptureModule returnJsonRpcWithError:result withMessage:@"failed to close unknown handle" withId:jsonRpcId];
                block(response);
            }
        }
    }
        ...
        if((int)handle == (int)_capture){
            ...
            if(block != nil){
                SKTResult result = SKTCaptureE_INVALIDHANDLE;
                NSString* response = [NativeCaptureModule returnJsonRpcWithError:result withMessage:@"handle is not the main Capture or a Device" withId:jsonRpcId];
                block(response);
            }
            return;
        }
        property = [NativeCaptureModule ConvertToPropertyFromJsonRpcProperty:jsonRpcProperty];
        [object getProperty:property completionHandler:^(SKTResult result, SKTCaptureProperty *property) {
            NSString* response;
            if(SKTSUCCESS(result)){
                response = [NativeCaptureModule returnJsonRpcWithHandle:handle withProperty: property withId:jsonRpcId];
            }
            else {
                response = [NativeCaptureModule returnJsonRpcWithError:result withMessage:@"failed to get a property" withId:jsonRpcId];
            }
            if(block != nil){
                block(response);
            }
        ...
        [object setProperty:property completionHandler:^(SKTResult result, SKTCaptureProperty *property) {
            ...
            if(SKTSUCCESS(result)){
                ...
                response = [NativeCaptureModule returnJsonRpcWithError:result withMessage:@"failed to set a property" withId:jsonRpcId];
            }
            if(block != nil){
                block(response);
            }
...
+(NSString*)returnJsonRpcWithHandle:(NSInteger)handle withProperty:(SKTCaptureProperty*) property withId:(NSInteger)jsonRpcId {
    NSString* jsonProperty = [NativeCaptureModule ConvertToJsonRpcPropertyFromProperty:property];
    NSString* jsonRpc;
...
+(SKTCaptureProperty*)ConvertToPropertyFromJsonRpcProperty:(NSDictionary*) jsonRpcProperty {
    ...
    switch(property.Type) {
        ...
        case SKTCapturePropertyTypeObject:
            ...
            socketCamContext = [NativeCaptureModule  InjectRootUiViewIfOverlayProperty:property.ID fromContext: socketCamContext];
...
+(NSString*)ConvertToJsonRpcPropertyFromProperty:(SKTCaptureProperty*) property {
    ...
    switch(property.Type) {
        ...
        case SKTCapturePropertyTypeUlong:
            ...
            break;
        case SKTCapturePropertyTypeArray:
            [jsonRpc appendFormat:@", \"value\": %@ }", [NativeCaptureModule ConvertToStringFromData:property.ArrayValue]];
            break;
        case SKTCapturePropertyTypeString:
...
+(NSString*)ConvertToJsonRpcEventFromEvent:(SKTCaptureEvent*)event fromCapture:(SKTCapture*)capture {
    ...
    switch(event.Data.Type){
        ...
        case SKTCaptureEventDataTypeArray:
            [json appendFormat:@", \"value\": %@ }}", [NativeCaptureModule ConvertToStringFromData:event.Data.ArrayValue]];
            break;
        ...
        case SKTCaptureEventDataTypeDecodedData:
            ...
            [json appendFormat:@"\"data\": %@,", [NativeCaptureModule ConvertToStringFromData:event.Data.DecodedData.DecodedData]];
        ...
        case SKTCaptureEventDataTypeLastID:
            ...
            break;
        default:
            NSLog(@"Default: Data Type Unfound.");
            break;
    }
    return json;
}
...
-(void) didReceiveEvent:(SKTCaptureEvent*) event forCapture:(SKTCapture*) capture withResult:(SKTResult) result{
    ...
    NSString* jsonEvent = [NativeCaptureModule ConvertToJsonRpcEventFromEvent: event fromCapture: capture];
